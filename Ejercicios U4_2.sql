...
--1. Se quiere saber ¿cuándo realizó su primer venta cada vendedor? y ¿cuánto fue el importe total de las ventas
--que ha realizado? Mostrar estos datos en un listado solo para los casos en que su importe promedio de vendido sea
...
--2. Liste los montos totales mensuales facturados por cliente y además del promedio de ese monto 
--y el promedio de precio de artículos Todos esto datos correspondientes a período que va desde el 1° de febrero al 30 de agosto del 2014. 
--Sólo muestre los datos si esos montos totales son superiores o iguales al promedio global.
...
--3. Por cada artículo que se tiene a la venta, se quiere saber el importe promedio vendido,
--la cantidad total vendida por artículo, para los casos en que los números de factura no sean uno de los siguientes: 2, 10, 7, 13, 22
...
select year(fecha) 'anio', MONTH(fecha) 'mes', DAY(fecha) 'dia',
        sum(cantidad)'cantidad total vendida del dia',
		sum(cantidad*pre_unitario)'total de ventas del dia',
		sum(cantidad*pre_unitario)/count(distinct f.nro_factura)'promedio importe'
    from facturas f join detalle_facturas df on df.nro_factura=f.nro_factura

	where f.fecha between '01/02/2014' and '30/08/2016'
	group by year(fecha) ,MONTH(fecha),DAY(fecha)
	having sum(cantidad) > (select sum(df2.cantidad)/count(distinct df2.nro_factura)
											from detalle_facturas df2)


--5. Se quiere saber el promedio del importe vendido y la fecha de la primer venta por fecha 
--y artículo para los casos en que las cantidades vendidas
--oscilen entre 5 y 20 y que ese importe sea superior al importe promedio de ese artículo.
